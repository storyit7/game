...
def draw_columns(n_people):
    ...
    # 고정 높이 12 단위를 기준으로 세로열 그리기
    for i in range(n_people):
        ax.plot([i, i], [0, 12], color='black', lw=5)
...
def draw_ladder(ladder, n_people, n_rows, markers=None):
    ...
    선 두께는 5, 마커 크기는 18로 설정합니다.
    markers: (x, y, color) 튜플 리스트
    """
    fig, ax = plt.subplots(figsize=(12, 12), dpi=100)
    # 세로줄 그리기
    for i in range(n_people):
        ax.plot([i, i], [0, n_rows], color='black', lw=5)
    # 가로줄 그리기
    for r, row in enumerate(ladder):
        y = r + 0.5
        for i, has_bar in enumerate(row):
            if has_bar:
                ax.plot([i, i+1], [y, y], color='black', lw=5)
    # 진행된 마커 그리기
    if markers:
        for (x, y, col) in markers:
            ax.plot(x, y, 'o', markersize=18, color=col)
    ax.set_xlim(-0.5, n_people - 0.5)
...
def main():
    ...
    if start_game:
        ...
        finished_markers = []  # 애니메이션 완료된 참가자들의 최종 위치
        
        # 1번 참가자부터 순차적으로 애니메이션 진행 (원 이동 속도: 0.1초 간격)
        for i in range(n_people):
        ...
        for i, path in enumerate(paths):
            final_col = path[-1][0] + 1  # 1-indexed 변환
